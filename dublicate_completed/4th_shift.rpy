...
label d_comission:
    $ save_name = u"Дубликат. Четвёртая смена - Комиссия"
    $ day_time()
    ...
    show d_us normal sport with dspr:
        ypos -0.05
    us_old "Виола, мы на озеро — то, что ближе к cтарому корпусу. Скажете Сёмке, чтоб не терял."
    hide d_us
...
label d_lake:
    $ save_name = u"Дубликат. Четвёртая смена - Озеро"
    $ day_time()
    scene black with dissolve
    ...
    show d_us normal sport with dspr
    us_old "Анатолий Сёмкину спину помассировал чуть-чуть. Потом Виола Сёмку посмотрела и сказала, что раньше воскресенья никуда не отпустит, что прочие коновалы — они только и могут, что таблетки пополам ломать. Так что три сеанса ещё."
    show us normal pioneer with dspr
...
label d_persunovs:
    $ save_name = u"Дубликат. Четвёртая смена - Персуновы"
    $ sunset_time()
    scene black with dissolve
...
label insomnia:
    $ save_name = u"Дубликат. Четвёртая смена - Бессонница"
    $ night_time()
    scene black with dissolve
    ...
    show d_sm sad2 pioneer at left with dspr
    ...
    show d_sm smile2 pioneer at left
    with squares
    play ambience ambience_camp_center_day
    ...
    show d_sm sad2 pioneer with dspr
...
label last_letter:
    $ save_name = u"Дубликат. Четвёртая смена - Письмо"
    $ night_time()
    scene black with dissolve
...
label routine_:
    $ save_name = u"Дубликат. Четвёртая смена - Рутина"
    $ night_time()
    scene black with dissolve
    ...
    show mi sad pioneer with dspr
    mi "А вот пионеры, Леночка — они вообще никак и ни на чём не играют и не хотят учиться."
    show mi smile pioneer with dspr
...
label promenade_:
    $ save_name = u"Дубликат. Четвёртая смена - Прогулка"
    $ night_time()
    scene black with dissolve
    ...
    nvl clear
    play music anewbeginning fadein 2
    show dct_us_old_camp_with_binoculars with dissolve
    $ renpy.pause (1)
    ...
    "Никто не видел бесконечного леса, начинающегося за лагерем: сперва светлая зелень листвы, дальше — отдельные острова тёмной хвои, ещё дальше — острова сливаются в сплошной хвойный массив до самого горизонта."
    ...
    "А Мику сначала испугалась того, что стала причиной семейного скандала, а потом положила метлу на асфальт и отошла к остановке, присев там на деревянную скамейку (два вкопанных в землю обрезка бревна и доска между ними)."
...
label cracks_:
    $ save_name = u"Дубликат. Четвёртая смена - Трещины в мироздании"
    $ night_time()
    scene black with dissolve
...
label artists_:
    ...
    show promo_text u"Сцена первая" with dspr:
        xpos 0.4 ypos 0.1
    pause
    show promo_text u"На лесной поляне вокруг костра сидят пионеры и вожатая. Одно место\nв круге пустует. Анфиса задумчиво перебирает струны на гитаре…\n\n{b}Лариса.{/b} Оксана Сергеевна, мы кого-то ждём? Почему мы сели так,\nчтобы одно место у костра осталось свободным?.." as promo_text_2 with dspr:
        xpos 0.05 ypos 0.2
...
label anticipation_:
    ...
    play music music_list["farewell_to_the_past_edit"]
    th "Славя так и не созналась, зачем она это сделала, но выглядит неплохо. Хуже точно не стало, а Славя стала казаться серьёзнее. Взрослее и солиднее."
