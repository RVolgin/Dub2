...
label dct_sixhour_dyn:
    ...
    $ set_mode_adv()
    show el normal pioneer close:
        zoom 1.095 xcenter 0.362 ycenter 0.55 rotate -8.9
    show dct_el_1_sleep:
...
label dct_transitions_dyn:
    ...
    play sound "<from 1.2 >dublicate/sounds/sfx/dct_sfx_chair.ogg" 
    ...
    show dct_int_dining_hall_table_day_front
    with blinds
    play sound "<from 0.5 >dublicate/sounds/sfx/dct_sfx_chair.ogg" 
...
label dct_fluctuation_dyn:
    ...
    play sound "<from 1.0 >dublicate/sounds/sfx/dct_sfx_chair.ogg"
    show dv shy pioneer close with dissolve_fast
...
label dct_debut_dyn:
    ...
    play sound_loop "<from 7.0 loop 7.0>dublicate/sounds/sfx/dct_sfx_steps3.ogg"
    play ambience ambience_forest_day fadein 1
    scene bg ext_path2_day
...
label dct_d7_d5_dyn:
    ...
    show sl sad pioneer as sl2 zorder 4:        # Дополнительно вырезаем юбку Слави и помещаем перед чемоданом
        corner1 (127, 467) corner2 (600, 720) xcenter 0.55 yalign 1.0
    ...
    stop ambience
    $ renpy.music.set_volume(1.0)
    play music "<from 32.0 loop 0.0>dublicate/Sounds/music/dreamers_of_the_day.ogg" fadein 3
    ...
    scene dct_sky_over_flowerbed_behind_library:
        size (1600, 1043) align (1.0, 0.0)
        ease 2.5 size (1280, 835) align (1.0, 0.5)
...
label dct_replacement_dyn:
    ...
    play music "<from 42.0 loop 0.0>dublicate/sounds/music/twisterium_eternal_love.ogg" fadein 7
    show d_jn:
...
label dct_system_errors_dyn:
    ...
    show d_sf sad pioneer with dspr
    "Семён замолчал. А через три минуты, перечитав ещё раз написанное, сложил листки бумаги пополам, спросил у Жени, есть ли у той в хозяйстве конверты, получил конверт из оберточной бумаги, спрятал в него написанное, собрался заклеить, но передумал."
    ...
    "\n\n{i}Надпись на конверте:{/i} {size=+4} Ульяна, если конверт не вскрыт — вскрой и прочти обязательно. С{/size}." 
    stop music fadeout 2.5
    window hide
    $ set_mode_adv()
    pause 1
    ...
    play sound_loop "<to 23.0>dublicate/sounds/sfx/dct_sfx_cry.ogg"  # Плач, но обрезанный, а то он там прям в рыдания переходит
...
label dct_dreamreality_dyn:
    ...
    th "А потом лампочка мигнёт в третий раз, шар сожмется ещё, и останется только Алиса, а всё прочее исчезнет."
    hide 3500_dv
    $ renpy.music.set_volume(1.0, delay=0, channel='ambience')
    show black with dissolve_fast:
        alpha 0.6
...
label dct_coin_final_dyn:
    ...
    show text "{color=#496463}{size=33}Конец книги «Монетка в фонтане»{/size}{/color}":
        align (0.5, 0.5)
    with dspr
