...
label dct_transitions_dyn:
    ...
    show d_mz normal glasses pioneer at left
    with dissolve
    ...
    # Невостребованные детали перекочевали обратно в ящик, который занял своё прежнее место под одним из верстаков."
    show sh normal pioneer close with dspr
    ...
    "\nПосле ужина нужно будет закрепить коробочку с усилителем на одном из флагштоков, отдать горнисту микрофон со шнуром и протянуть провод от усилителя к радиорубке.{w} Почему-то вожатая настаивала на том, что горнист должен подавать сигналы обязательно с площади."
    window hide
    $ set_mode_adv()
    window auto
    ...
    window hide
    play sound [ "<silence 0.2>", sfx_uliana_jumps_down ]
    show us normal sport:
        ease 0.6 xpos 0.5
...
label dct_fluctuation_dyn:
    ...
    # th "Вот и прекрасный повод. Пойду и помогу."
    
    scene bg int_library_day
    show mz normal pioneer
    with dissolve
    "И вот, когда Электроник где-то через час после завтрака в нервной трясучке прибежал к библиотеке и предложил свою помощь… Да что теперь вспоминать? В общем, отлуп, бессмысленный и беспощадный. «За книгами приходи, раз уж записывался», — сказано было вдогонку, равнодушно, как к стотысячному посетителю за сегодня. Это если без подробностей, которые вспоминать не хотелось."
    $ set_mode_adv()
    ...
    pause
    window auto
    "Идти в столовую и встречаться там с Женей не хотелось. Слишком больно."
    ...
    play ambience ambience_camp_center_day fadein 1
    "Шурик сходил на обед, удивился, не встретив там Сыроежкина, вернулся в кружок и засел с тетрадью за кустами барбариса у задней стены здания клубов, чтобы составить план работы кружка кибернетики."
    "Ничего не получалось, хоть плачь, как он ни пытался сосредоточиться."
    ...
    # "Шурик не отвечает, только молча смотрит на них обеих — на то, чтобы молчать, у него ещё хватает сил."
    show d_jn_old normal
    ...
    # pause 0.2
    "С площади доносилось только приглушённое «бум-бум-бум». Верхний свет Женя погасила, остался только тот, что проникал снаружи от уличных фонарей, и настольная лампа. Но книжка не шла."
    ...
    show dv grin pioneer far with dissolve_fast
    dv "Не прогоню. Приглашай… племянник.{w} Но с условием. Жени и Ленки не будет, скорее всего, но чтобы Мику и Сашу пригласил обязательно."
    ...
    # nvl clear
    "С танцами вообще получилось неплохо. Алиса — как обычно, чуть насмешливая, Саша — благодарно улыбнувшаяся, танцевавшая на пионерской дистанции и в конце сказавшая: «У тебя есть способности, если хочешь, приходи к Мику в кружок, подучим», Мику — «Максимочка, конечно же можно!». Всех этих девочек как будто знал и со всеми как будто был в хороших отношениях уже давно-давно, с детского сада."
    "Ещё раз Алиса. Катя фыркнула и отправилась танцевать с Витькой, соседом по домику."
...
label dct_disposition_dyn:
    ...
    show d_el think2 pioneer close as el:
        pause 1
        linear 0.8 alpha 0
    ...
    with slideawayup
    th "В библиотеку?{w} Сейчас, когда голова на время освободилась от забот, очень хочется увидеть Женю."
    hide 3500_el with dissolve
    th "Я только возьму что-нибудь почитать." 
    
    scene bg ext_library_day
    show el scared pioneer far at cright
    with pushleft
    ...
    # "В общем-то, каждый любитель посидеть-почитать был понятен Жене и не вызывал у неё ни агрессии, ни испуга."
...
label dct_replacement_dyn:
    ...
    "\nСенечка, я сегодня чуть с ума не сошла. Сижу в кружке у себя, пытаюсь играть — под дождь так хорошо играется, а вижу себя обезьянкой в клетке.{w} У меня истерика случилась, а Сашенька с доктором меня валерьянкой и ещё какими-то таблетками отпаивали. Поэтому я замороженная сейчас, не обращай внимания, это пройдёт.{w} А знаешь, почему я с ума не сошла? Из-за той сказки, что ты принёс. Я подумала, что если моя сестра способна творить, то, значит, и я сумею.{w} А обезьянка — нет, обезьянки творить не умеют. А ещё я за твою историю с Микусей уцепилась. Я и эту историю тоже теперь помню. Да, Микуся там, в мире наших снов, и здесь она не появится никогда, но свою память она нам подарила.{w} Сенечка, ведь если она была счастлива, пусть даже так коротко и такой ценой, значит, и для меня где-то запасён в мире кусочек счастья?!"
    stop music fadeout 1.5
    window hide
...
label dct_system_errors_dyn:
    ...
    # "Шурик стоял, замерев и уткнувшись лбом в засиженное мухами стекло, и только лёгкое шевеление при каждом вдохе выдавало в нём живого человека."
    el "\~ Переживает. \~"
    ...
    # th "Где та Евгения, которая орала на Серёжу в автобусе в день приезда?"
    ...
    $ sunset_time()                        # Прописано, чтобы при обратной перемотке цвета не сбивались.
    $ persistent.sprite_time = "sunset"
    th "В этом лагере что-то можно долго хранить в секрете?"
    show black as black2 with dspr
    window hide
    pause 1
...
label dct_dreamreality_dyn:
    ...
    with dissolve
    # "Пять минут спустя после прихода Алисы на заднем ряду лавок устроились Максим и Саша."
    # window hide
    # show mz normal pioneer:
    ...
    # th "Сейчас она опять начнёт издеваться."
...
label dct_horizontal_transport_dyn:
    ...
    show anim prolog_1:
        alpha 0
        linear 0.5 alpha 1
    "Всё это началось в прошлом цикле, после той спасательной экспедиции, которую мы с Алисой предприняли в поисках тебя и Ульяны."
    "Я тогда тоже, как ты, решила, что наш мир не может существовать.{w} Только я решила, что он не компьютерная модель, а чья-то фантазия."
    "А потом меня заинтересовал человек, придумавший наш мир.{w} Какое-то время я отбрасывала от себя эту мысль, но снова и снова к ней возвращалась.{w} А дальше приехал мой Семён, и стало уже не до того."
    "В конце цикла, когда мы ехали в автобусе и мой Семён уже уснул, я подумала, что, может, если я пойму этого человека, то я тогда пойму и то, как мне разбудить моего Семёна — ведь он такой же придуманный, как и мы все."
    "И я попыталась на основе того, что знаю о мире, представить себе придумавшего его автора.{w} Так у меня в альбоме появился первый «портрет неизвестного»."
    "А потом я поняла, что не может один человек столько выдумать и в голове держать."
    ...
    "Алиса ещё сказала, что эти прозвища называются «ники»."
    nvl clear
    "А потом ты унёс мой рисунок в тот, другой лагерь, а через два часа появилась недовольная тем, что её оторвали от каких-то своих дел, Ульяна-маленькая и передала новую работу Мику из того лагеря и её просьбу: «Что-нибудь с этим сделать»."
    ...
    "Ну, не хорошо, конечно, Земля — она вон какая большая, а пионеров вон как мало, но хоть чуточку, но лучше."
    "И вот у нас всё встало на свои места.{w} Никакая мы не модель. Никто нас не придумывал. Есть наши лагеря — Сеть, как вы с Ульяной их называете, есть материнские миры и есть Земля."
    "И всё это связано — через людей.{w} И там, и там, и здесь."
    "И вот мы сейчас переписываем рассказик.{w} То есть переписывают Мику из того лагеря с нашей Алиской, а я так, на подхвате — почитать, покритиковать, иллюстрации сделать.{w} И Ульянка-маленькая, она — наш почтальон."
    show anim prolog_1:
        alpha 1
        linear 0.5 alpha 0
    window hide
    ...
    "Неизвестно зачем, проходя мимо музыкального кружка, они решили заглянуть в окно.{w} Неизвестно зачем, увидев Мику за роялем, они решили заглянуть на минутку и поздороваться." # Потому что даже то, что в открытое окно изнутри не доносилось ни звука, их не насторожило.
    ...
    stop sound_loop fadeout 1
    play ambience ambience_dining_hall_full fadein 1
    show int_dining_hall_people_day with pushleft
    ...
    show dv smile pioneer at cleft:
        ...
        pause 1.0
        linear 1 alpha 1
    "Семён с Ульяной и Алиса с ними что-то спокойно обсуждали." # Немного отсебятинки
    hide d_sf
    hide d_us
    hide dv
    with dissolve
    ...
    show un smile pioneer far:
        ...
        pause 0.8
        linear 1 alpha 1
    show mi grin pioneer far behind sl:
        xcenter 0.4 ycenter 0.74 alpha 0
        pause 1.2
        linear 1 alpha 1
    "Саша, Максим и Лена с заботой смотрели на Мику, необычно осунувшуюся и бледную."
    "Но она улыбалась и ела с аппетитом, как будто выздоравливая после тяжёлой болезни."
    hide mi
    hide sl
    hide d_ma
    hide un
