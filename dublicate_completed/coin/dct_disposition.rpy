...
label dct_disposition:
    ...
    dv "И что? Робота делать не будете? Ну вы даете, вы не заболели там? Надо будет зайти и глянуть."
    scene dct_int_clubs_male2_night_zoom
    show dct_dream_veil at shiver
    with dissolve
    "Разработать конструкцию ультразвуковых излучателя и приемника (А что делать? Готовых то нет, а из всей литературы только пара подшивок журнала «Радио» в кружке, да десятка два разрозненных номеров «Моделиста-Конструктора» и «Юного техника» в библиотеке)."
    window hide
    ...
    "Забегал, не обращая ни на кого внимания, в библиотеку и там молча шел к стеллажу с журналами, перечитывал их на двадцать пятый раз, делал выписки и опять бежал в клуб."
    ...
    "Принципиальная схема на листе оранжевой миллиметровки была пришпилена к дверце шкафа поверх чертежа авиамодели, горка деталей извлеченных из ящика и выпаянных из некондиционных плат лежала на втором листе миллиметровки и сейчас Шурик, прозванивая, раскладывал их по кучками: резисторы отдельно, транзисторы отдельно и так далее."
    scene dct_int_clubs_male2_night_zoom
    ...
    sh "Нет, иди-иди, а то мне еще подумать надо."
    ...
    th "А вдруг она о чем-нибудь меня спросит?"
    ...
    # un "Попросила подменить ее до обеда. Ты что-то хотел взять? Возьми, я запишу."
    # show un shy pioneer close with dspr
    # "Лена быстро глянула на кибернетика своими зелеными глазищами."
    ...
    with dissolve
    el "П-привет. А где Женя?"
    un "Попросила подменить ее до обеда."
    ...
    show d_mz sad pioneer glasses far with dissolve:
        xcenter 0.8
    window auto
    "Лена утром зашла в библиотеку без всякой глобальной цели, просто поменять книжку, но зацепилась взглядом за Женю. Та грустила в углу зала, за своим столом-конторкой, чем-то напоминая нахохленную птицу под дождем."
    ...
    "А потом еще в разговоре, пока Женя записывала в формуляр книгу, выяснилось, что она со вчерашнего дня сидит безвылазно в библиотеке, с перерывами только на еду и сон."
    "И понадобились все искусство убеждения, и умение выдавать свои мысли за чужие желания, которыми обладала Лена, чтобы уговорить Женю просто сходить прогуляться по лагерю."
    show d_mz sad pioneer at right with dissolve_fast
    mz "Лена, а если кто-то придет..."
    un "Тогда я выдам ему нужное издание и заполню формуляр не хуже тебя."
    mz "Я только..."
    un "Хорошо-хорошо. Я подежурю. Только ты не долго."
    window hide
    ...
    "Лена погасила настольную лампу, отодвинула тяжелую портьеру у себя за спиной и впустила в библиотеку солнечные лучи.{w} Взвесила в руке выбранную книжку: на пару дней чтения хватит. Бросила взгляд на библиотечные стеллажи."
    th "А ведь рано или поздно книги кончатся. Все, что было здесь хорошего, я уже прочитала. Сейчас читаю средненькое. А потом?"
    extend " Может Семен что-то принесет?"
    "Семен с Ульяной ушли сегодня утром и клятвенно обещали вернуться завтра часам к десяти, оставалось надеяться, что они вернутся не с пустыми руками."
    "По крайней мере, та пьеса, что они принесли в прошлый раз, была проглочена Леной за сутки, потом еще три раза перечитывалась и породила массу карандашных рисунков-иллюстраций, самый доведенный до ума из которых сейчас ехал обратно, как подарок автору пьесы. Та, другая Мику — автор пьесы, обладала еще и литературным талантом."
    "А еще, интересно было осознавать, что где-то есть очень похожие на тебя Лены, встретиться с которыми, по словам того же Семена, почти невозможно."
    ...
    hide dct_dream_veil
    with dissolve_fast
    "{cps=0}А еще, интересно было осознавать, что где-то есть очень похожие на тебя Лены, встретиться с которыми, по словам того же Семена, почти невозможно.{/cps} Можно только во сне."
    ...
    "Максим удивленно разглядывал Лену, стоя в дверях."
    ...
    "И, не дожидаясь ответа, пошел шарить по стеллажам."
    ...
    show d_ma grin pioneer behind dct_int_library_standing_desk_front:
        ...
        pause 2
        linear 1 alpha 1
    "Подумала Лена обреченно, когда из-за стеллажей показался Максим, весь в пыли и с номерами «Знание-сила» в руках."
    ...
    un "Ты только что разгромил половину библиотеки ради пары журналов.{w} Возьмешь их? Давай я запишу."
    show d_ma grin2 pioneer with dissolve_fast
    d_ma "Не, не половину. Четвертую часть в дальнем углу, куда никто не заглядывает."
    ...
    show mt angry panama pioneer close:
        ...
        linear 1 alpha 1
    with dissolve
    "Тем более, если сейчас пойдешь в домик, а тебе вожатая попадется навстречу, то неизбежны вопросы: «Максим, почему на площади бумажки? Это теперь твоя подотчетная территория. — Или. — Максим, почему бездельничаешь? Или занимайся, или марш со своим отрядом в лес, шишки и желуди для конкурса \"Умелые руки\" собирать!»"
    play ambience ambience_library_day fadein 1
    scene bg int_library_day
    ...
    d_ma "Тогда понятно.{w=0.5} Да просто даже Алиса никогда не будет взрослого, пусть и хорошо знакомого человека, Сенькой называть.{w} И она всегда так заботится о Семене, а Ульяна видит это и только улыбается."
    show un serious pioneer with dspr
    un "Ты наблюдательный..."
    show d_ma grin2 pioneer with dissolve_fast
    "Лена думала, как закруглить разговор. В конце-концов Максим пока еще только не проснувшийся кандидат в старший отряд, и разъяснять тонкости взаимоотношений Рыжих с Семеном постороннему человеку не хотелось."
    ...
    show el normal pioneer close:
        ...
        linear 1 alpha 0
    with dissolve_fast
    "Поздоровался, Лене показалось что разочарованно, и пошел выбирать себе книжку."
    "Лена вспомнила про «Машину для чтения памяти» и, решив застолбить себе место среди подопытных, стала ждать, когда Сергей вернется к конторке."
    window hide
    stop ambience fadeout 1
    scene black with dissolve
    ...
    extend " Нет, потеряли бы, керамике все равно остывать до утра вместе с печью."
    stop ambience fadeout 1
    play sound_loop ambience_camp_center_day fadein 1
    scene ext_clubs_day with dissolve
    play sound sfx_unlock_medpunkt_door
    "Шурик запер клуб и, огибая здание, пошел на свое вчерашнее место."
    stop sound_loop fadeout 1
    ...
    with dissolve
    "Прежде чем начинать зондировать собственный мозг, главному кибернетику предстояло разобраться, что ему известно об этих своих сновидениях и голосе в голове, и записать данные самонаблюдения и самоанализа в рабочую тетрадь, чтобы иметь точку отсчета."
    "Сыроежкин уже согласился быть вторым — контрольным экземпляром. Неплохо было бы, для статистики, набрать еще добровольцев, но Шурик опасался возможного риска. В любом случае, как настоящий ученый из любимых книг, Шурик твердо знал, что начнет с себя."
    "Председатель кружка кибернетики открыл личную рабочую тетрадь и на первой чистой странице написал:{w}\n{i}Дневник самонаблюдения. Начат в четвертый день смены. Данные по предыдущим дням восстановлены по памяти{/i}."
    ...
    "Поднял глаза. Перед Шуриком стояла девочка, лет, наверное, девяти, не больше. Светлые глаза, стрижка такая, что не понять, коротко стриженная перед тобой девочка, или обросший мальчик. Если бы не юбка, аккуратность в одежде и маленькие сережки-гвоздики в ушах."
    th "Сергея? А! Это Сыроежкина что ли?"
    sh "Нет, занятия закончились и он ушел. Поищи его в библиотеке."
    ...
    show el normal pioneer far at right:
        ...
        pause 1.2
        linear 0.8 alpha 1
    "Шурик занес тетрадь в кружок и, встретив на крыльце пришедшего за ним Сыроежкина, отправился в столовую."
    window hide
    scene black with dissolve
    stop sound_loop fadeout 1
    ...
    th "Почему Алиса куксилась, что все, из цикла в цикл, одно и тоже? Может, она просто видеть не умеет? Не знаю. И пионеры и природа, каждый цикл, но чуть-чуть разные."
    th "Вон, куст подсыхает, прошлый цикл зеленый стоял, а в этом — листья теряет. А здесь за четыре дня новую тропинку протоптали, интересно — приживется или нет? А уж люди как меняются... Это просто надо увидеть."
    "Лена шла из столовой к себе в домик и размышляла."
    scene bg ext_house_of_un_day with dissolve
    th "Я, наверное, порисую сейчас. Портрет Саши начну, а то сколько времени живем в одном домике, а так Саши у меня и нету."
    stop ambience fadeout 1
    play sound_loop ambience_int_cabin_day fadein 1
    play sound sfx_open_door_1
    scene bg int_house_of_un_day with slidedown
    "Саши дома не оказалось."
    th "Или возится с мелкими, или еще где-то, хоть у Мику в кружке, в этом цикле они сблизились раньше обычного."
    "Купальный сезон официально открывался в четверг, а в следовании инструкциям Саша была похожа на Сережу Сыроежкина, так что на пляже ее точно не было."
    th "В следующий раз."
    "Лене пришлось в снова отложить мысль о Сашином портрете."
    th "Взять этюдник и уплыть на остров? Причем, на малопосещаемый остров Длинный."
    extend " Или уйти с этюдником на остановку и, в очередной раз, нарисовать дорогу, убегающую в поля?"
    "Лена взвесила в руках этюдник и поморщилась — тяжелый. Да и не очень то и хотелось."
    ...
    "Женя не договорила и, все так же не глядя на Лену, пожала плечами."
    ...
    "Еще на полчаса девочки погрузились в молчание, прерываемое шелестом бумаги и редкими вздохами. Да иногда снаружи доносились голоса пионеров: по Плану мероприятий у среднего отряда было сейчас свободное время, чем они и пользовались, устроив беготню по всему лагерю."
    th "А ведь она ждет, что Сергей заглянет, а тот боится и где-то прячется."
    extend " Зря девочки посадили Сергея на моё место, и зря я на это согласилась. Нужно бы сейчас все рассказать, но Женя не поверит, решит, что я Сергея выгораживаю. С ее-то точки зрения все было так, как она увидела."
    hide mz
    ...
    un "Это девочка одна, знакомая нашего физрука, сказку написала. И Семен попросил, чтобы я к ней иллюстрации сделала."
    ...
    "Лену Женя уже отнесла к числу безопасных, подвоха от нее не ждала, поэтому выгонять из библиотеки не стала, а разрешила остаться и посидеть-почитать. В общем-то, каждый любитель посидеть-почитать был понятен Жене и не вызывал у неё ни агрессии, ни испуга."
    ...
    mz "Да мне как-то все равно. Если нужен мой голос, можешь передать, что я — за."
    ...
    "После ухода Лены Женя еще раз обвела глазами библиотеку, увидела журналы на полке с прочитанной и возвращенной литературой."
    ...
    "«Родионов Максим, 14 лет, {s}средний{/s} отряд». Над зачеркнутым словом «средний» рукой Лены было написано «старший»."
    scene bg int_library_day
    show d_mz smile2 pioneer glasses close
    with squares
    mz "Любит ли читать, я еще не знаю, но умеет, это точно."
    "Проскрипела вслух заведующая библиотекой, чуть улыбаясь и пристраивая формуляр в общую шеренгу, между Персуновым Семеном и Тихоновой Еленой."
    show d_mz normal pioneer glasses close with dissolve_fast
    "Теперь осталось только вернуть журналы на место и можно запирать библиотеку на перерыв. Или не запирать — идти все равно некуда."
    "Женя взяла журналы, подержала их в руках и положила обратно на полку."
    show d_mz sceptic pioneer glasses close with dspr
    "Потом взяла самый верхний и быстро пролистала его. Нет, все верно, вон и библиотечный штамп на месте. Но все же."
    show d_mz surprise pioneer glasses close with dspr
    "Женя подошла к каталогу, выдвинула несколько ящичков и умело пробежалась по картотеке, потом проделала это еще раз, читая каждую карточку, вынимая карточки из ящичков и проверяя — не завалилась ли какая-нибудь карточка, выпав из общего ряда."
    show d_mz amazed pioneer glasses close with dspr
    "Потом проделала тоже самое уже с каждым ящичком, а не только с теми где была периодика и издания на букву З. Ничего нового, никаких поступлений, но в фондах библиотеки «Совенка» не числилось ни одного номера журнала «Знание-сила». Не смотря на библиотечный штамп на этих самых журналах."
    ...
    "Девушка глянула на календарик, притаившийся под оргстеклом, покрывающим ее стол. Согласно календарику на дворе стоял июнь 1987 года."
    show d_mz sceptic pioneer glasses close with dissolve_fast
    th "Кажется придется прийти на сбор отряда."
    th "Кажется, придется спросить у новенького, где он берет журналы? Может он и книг хороших оттуда же натаскает?"
    window hide
    stop ambience fadeout 1
    scene black with dissolve
    ...
    th "Янка... может быть, если бы Ульяна была помоложе лет на пять, то так она себя бы и вела. Степан? Не могу себе представить, неужели Семен был таким?"
    th "Нет, в нем конечно живет семнадцатилетний пионер, это не вооруженным глазом видно, но и двадцатипятилетний заместитель начальника лагеря тоже в нем."
    "Саша опять вернулась к началу и посмотрела на первую страницу."
    th "Как, когда Мику успела все это сочинить? Она же сама говорила в день приезда, когда знакомились, что она впервые в СССР, а ее папа с мамой сейчас едут на поезде из Владивостока в Москву, с остановками в крупных городах."
    th "Но вот же автор: Мику Хацуне, а вот посвящение: „Сенечке, которому… и Ульянке…“, — и подпись японскими закорючками. То есть, Мику знала Семена, когда тому было семнадцать? А самой Мику, получается, восемь-девять? Ничего не понимаю, надо будет спросить для начала у Лены."
    ...
    "Второй портрет, парень, похожий на Семена, наверное, таким он и был в семнадцать лет, только глаза не семнадцатилетние, а взрослые и какие-то уставшие что ли, но понимаешь, что он сейчас улыбнется этими уставшими глазами, и скажет: «Все ерунда, прорвемся!»"
    "Третий портрет, тот очень позитивный, на нем Семен и Ульяна, сидящие рядышком, рука в руке, на крыльце этого самого домика, оба улыбаются, оба счастливы, как раз и видно, что подросток из заместителя начальника лагеря никуда не делся."
    ...
    show d_oz smile pioneer at cright:
        yalign 0.27
    with dissolve
    "Вторую половину дня Сыроежкин провел в компании милой девушки Оксаны."
    hide el
    hide d_oz
    with dissolve
    ...
    el "Ты, когда острым ножом режешь — рука просто идет за ножом и все, и не надо лишних усилий делать. Просто следи, чтобы пальцы на пути у лезвия не оказались."
    show el serious pioneer with dissolve_fast
    extend " А когда режешь тупым, то ты напрягаешься и у тебя нож обязательно сорвется и попадет по пальцам, как бы ты их не берегла."
    hide el with dissolve    
    "Ну и все это перемежалось другими разговорами."
    show d_oz n pioneer at cright with dissolve:
        yalign 0.27
    d_oz "Вот знаешь, Сережа, мне кажется, ты ей нравишься."
    ...
    hide d_el
    with dissolve
    "Потом искали вдвоем подходящий кусок коры и Оксана пыталась выстругать хоть что-то, похожее на кораблик."
    ...
    el "А ты хочешь, чтобы он просто на полке стоял или, чтобы его еще и на воду спустить можно было?"
    "И тут же:"
    el "Знаешь, напиши ему письмо, небольшое, чтобы упаковать можно было.{w} В клубе была папиросная бумага, вот на ней завтра и напиши. Потом в полиэтилен запаяем и внутри корабля спрячем.{w} Если все так, как ты говоришь, он обязательно полезет разбирать твой подарок. Там письмо и найдет. Может, это сработает."
    "И снова взялись за ножи."
    el "А почему ты все своими руками должна делать?"
    "..."
    el "Что значит, иначе цикл не переживет?"
    ...
    "А Электроник еще посидел на берегу, улыбаясь и думая о том, какая это славная девочка и о ее словах, что он Жене нравится."
    play sound dct_sfx_horn_dinner_through_loudspeaker
    $ sunset_time()                       # Прописано, чтобы при обратной перемотке цвета не сбивались.
    $ persistent.sprite_time = "sunset"
    "Дождался горна и уже тогда встал, отряхнулся и пошел сперва в кружок за Шуриком, а оттуда на ужин."
    ...
    with dissolve
    window auto
    "Алиса была помощницей вожатой в этом лагере исключительно из-за отсутствия альтернатив и помощница вожатой из нее вышла плохая, но какие-то привычки въелись и их уже не вытравить. Вот и сейчас она перебирала в памяти прожитый день, анализировала свои ошибки, делала выводы и строила планы на завтра."
    ...
    me "Да Алис. Точка стабильная и еще долго продержится. Да не кисни, Рыжая. Завтра же придем назад!"
    ...
    sa "Да, не пойму я её что-то. Вчера просилась, а вечером, после дискотеки, я зашла к ней, напомнить, а та уже передумала.{w} Еще и смотрит обиженно, и глаза красные, будто плакала."
    ...
    "И убежала, только легкие шлепки подошв о покрытие беговой дорожки были слышны за спиной."
    ...
    dv "Ольга Дмитриевна, помните, в первый день сестренка Ульянина отметилась, которая отдыхает тут недалеко. Вот, кажется ее навестить."
    ...
    "А еще завтра открытие купального сезона и физруки на пляже были бы ох как уместны."
    show 3500_dv sad night2 0pt with dissolve_fast
    th "Надо будет на следующий цикл убедительно попросить их не уходить вдвоем."
    show 3500_dv grin night2 0pt with dissolve_fast
    extend " Стоп.{w=0.5} Они же завтра к одиннадцати обещали вернуться, так что все нормально."
    ...
    th "Привыкаю постепенно, вот уже и ее кровать своими вещами занимаю, и белье на эту кровать брать на складе не стала. Но все равно вечерами тоскливо. Хорошо хоть Ульянка каждый вечер специально поболтать заходит."
    show 3500_dv normal night2 0pt with dissolve_fast
    ...
    show dct_dream_veil at shiver
    with dissolve_fast
    us "Алиса, если бы ты в тот раз промахнулась, то Семен прошел бы сквозь наш лагерь, мимо и дальше."
    hide d_us
    hide dct_dream_veil
    show 3500_dv shy night2 0pt at left with dissolve_fast:
        ycenter 0.83
    with dissolve_fast
    th "А эта «сестренка» Ульянкина тоску разбередила."
    th "Уговорить бы ее здесь остаться, так в ее лагере своя Алиса есть, той тогда совсем плохо будет. Хуже, чем мне в прошлом цикле."
    show 3500_dv normal night2 0pt with dissolve_fast
    th "А у меня уже какие-то просветы появились. Вроде Макса того же."
    show 3500_dv laugh night2 0pt with dspr
    extend " Правильно Мику про него сказала, что он тычется всюду с любопытством и дружелюбием, как молодой пес."
    ...
    th "Интересно, Ленке тоже самое снится?"
    # "Ещё успела подумать Алиса, перед тем, как уснуть окончательно."  # Опускаем
    stop music fadeout 3
    show black:
        alpha 0
        linear 3 alpha 1
    th "Она ведь та еще партизанка, будет молчать, пока совсем плохо не станет."
