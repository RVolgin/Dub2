...
label dct_coin_final:
    $ save_name = u"Дубликат. Монетка в фонтане -- Финал"
    $ day_time()
    ...
    my "Юля, ты же знаешь, что со мной это не сработает. Тем более, в этом варианте «Совенка»."
    ...
    uv "Это все потому что я не материализовавшееся подсознание и не ходячий портал, а всего-лишь кошка-мутант."
    ...
    uv "Скоро Славя придет, не люблю, когда меня видят. Я, в конце-концов, как там у тебя: «Кошкодевочка, легенда лагеря», — и должна являться избранным и в критические минуты."
    show uv smile far with dspr
    uv "Еще увидимся, пока."
    my "Мяу!"
    ...
    "Или смартфон?{w=0.5} Или МР3-плеер?{w=0.5} Под моим взглядом старенькая кнопочная Нокия начинает увеличиваться в размерах, экран у нее наползает на кнопки…"
    "Кручу головой, «Отставить!». Телефон возвращается к привычному облику. М-да, и вот пошел бы я такой за Юлей, я бы натворил там дел, в лагере-то."
    "А так…"
    ...
    my "Славюшка, ты же давно уже проснулась. А все повторяешь так, как тебя научили."
    ...
    sl "Ты… Ты… Это ты приехал?! Так что ж стоишь, пойдем скорее."
    ...
    th "Здесь я еще сдерживаюсь, а внутри, боюсь что не смогу. И дело не только в пионерлагере, но и, по закону отдачи, во мне самом."
    ...
    "И бегом, в темпе приличном Ульянке-маленькой, а не помощницы вожатой, скрывается за воротами."
    th "Конечно знаю, конечно подойду, за тем я тут и оказался."
    

    scene dct_ext_bus_road
    show dct_ext_bus_road_wheat_Maya
    with pushright
    "Присаживаюсь на лавочку, до поляны двадцать минут хода, так что полчаса свободного времени у меня еще есть."
    "Или я лучше даже прилягу. Устраиваюсь у ног правого пионера, в удивительно не пыльной и мягкой здешней траве, закидываю руки за голову и лежу, разглядывая облака."
    "Как их называют? Цирусы, если я правильно вспомнил. Когда я еще здешнее небо увижу?"
    ...
    "Пионер. Визитер третий, виртуальный. Через него небо просвечивает, значит — виртуальный."
    ...
    my "Каждый твой шаг сужал тебе поле для маневра и ты, в итоге, оказался там, где оказался."
    my "И вот за последствия того, что ты загнал, или позволил загнать, себя в эту воронку, ты и должен нести ответственность."
    my "А ведь и нужно было тебе, когда все началось, всего-то, встать на чужое место и посмотреть оттуда."
    pi "Мудрено.{w=0.5} Но, прощай."
    show pi:
        linear 2 alpha 0.0
    my "Прощай."
    ...
    "Перевожу взгляд с облаков на средний план. На кусте барбариса сидит птичка, не разбираюсь я в них. Мелкая, чуть побольше воробья, очень аккуратное тельце, небольшой тонкий клюв, по серо-коричневому тельцу желто-коричневые продолговатые пятнышки."
    ...
    us "Его все уже ждут, а он тут разлегся!"
    ...
    with squares
    "И мы идем на костровую поляну, не заходя в лагерь. Мне же в лагерь нельзя, я помню."
    ...
    extend " Вот теперь каждый грызет свою половину."
    show us normal sport with dspr
    "Пока Ульяна занята яблоком, я кручу головой по сторонам. Вот так, вживую увидеть все, когда еще удастся?"
    "Куча мелких деталей, вроде муравейнка у самой тропы или лесных цветов, или заросшей просеки, уходящей неизвестно куда. И все это раньше не замечалось или проскакивало мимо сознания."
    ...
    "И пахнет грибами и хорошо бы проверить на этот счет во-о-он тот косогор."
    ...
    show us normal sport with dspr
    th "Кто-то еще крадётся возле нас вдоль тропинки, я это чувствую."
    show dct_uv_ears_bush:
    ...
    us "Расскажи, как ты живешь?"
    my "Нормально живу, Уля. Все вроде бы нормально, и проблем не много и не мало, так среднее количество. И близкие люди есть, которые меня понимают, и которых понимаю я. Но вот узнал о вас, и захотел познакомиться."
    show us smile sport with dspr
    us "И напридумывал всякого."
    my "Нет. Все что можно придумать, где-то уже существует. В моем мире придуманные вы, в вашем мире — я. Где-то еще кто-то третий."
    show us normal sport with dspr
    my "Так что я просто подсмотрел, как вы живете. Было трудно, но кое-что я увидел."
    show us sad sport with dspr
    ...
    d_gg "Рыжая белка, зачем тебе моё имя?"
    show us smile sport with dspr
    us "Надо!"
    d_gg "Ну хорошо, скажу перед уходом."
    ...
    show d_to m smile:
        ...
        pause 1
        linear 2.0 alpha 1.0
    "Дети лет семи.{w=0.5} Темноволосая девочка нацепила фонендоскоп и с серьезным видом слушает полноватого мальчишку, задравшего по такому случаю рубашку."
    ...
    "Я взвешен, оценен и признан безопасным."
    th "Интересно, что бы стал делать этот пузанчик, если бы решил, что я представляю опасность? Забил бы фонендоскопом? Самый первый цикл в младшем отряде и старые привычки еще не стерлись до конца."
    hide d_cs
    hide d_to
    with dissolve2
    d_gg "Что-то много народу, Славя."
    sl "Сколько смогло и сколько захотело. А захотели все, кто понял, что происходит, про кого ты упомянул, и даже просто подумал, и еще сверх того."
    sl "Ну, будешь речь толкать или пойдешь к костру?"
    th "К костру, конечно. Ишь чего удумала — речь ей толкать."
    hide d_sl with dissolve
    "Мне освобождают место рядом с Алисами, дают в руки уже очищенную печеную картошку, ставят рядом кружку с заваренной смородиной."
    ...
    dv "Значит все-таки один из Семенов?"
    ...
    d_gg "Алис, ты ведь та, которая помощница вожатой, в лагере физруков и бабы Глаши?"
    ...
    d_gg "Нет, не Семен. Я просто воспользовался на три-четыре часа его телом, пока он спит в автобусе."
    d_gg "Надо будет вернуть."
    show dv grin pioneer:
        xzoom 1.0
    hide dv_4_badge_mirror
    with dspr
    dv "А я то думала!"
    d_gg "Нет. И даже близко не попала."
    "А теперь моя очередь спрашивать."
    d_gg "Алиска, а ты поедешь с концертами по лагерям?"
    ...
    d_gg "Между прочим, квартира в двухэтажке, сорок шестого года постройки, была на первом этаже, в ней было три комнаты, кухня и туалет с ванной. Только вот, чтобы помыться в горячей воде, приходилось топить дровяной титан и плита на кухне тоже оставалась дровяная. В ваш дом газ так и не провели. Это вам в копилку ваших общих воспоминаний."
    ...
    "А я начинаю изучать печеную картошку. Дегустировать. Ее и смородиновый чай."
    ...
    d_gg "Привет. Ты одна здесь?"
    un "Да, остальные не могут. Пока не могут. Жаль."
    d_gg "Они проснутся, Лен, обязательно."
    ...
    d_gg "Ну к вам-то заглядывал не я один. Так что имена я уже знал. Так, несколько имен добавил в копилку и всё."
    show un smile3 pioneer close with dspr
    un "Ну вот, а мы, со здешней Мику вдвоем, заглянули к тебе. Не бойся, обещаю тебе, что все подумают, что это выдуманное нами имя."
    show un serious pioneer close with dspr
    un "Скажи, как ты думаешь, когда мой Семен проснется?"
    d_gg "Скоро Лен. Не в этом цикле, но очень скоро."
    show un surprise pioneer close with dissolve_fast
    d_gg "Он зайдет в лагерь, повернет к голову к клубам, увидит тебя, и скажет одними губами: «Ленка! Я прорвался!», но ты его прекрасно услышишь и бросишься к нему на шею, завизжав так, что перепуганные кибернетики выскочат на крыльцо."
    show un cry_smile pioneer close with dspr
    d_gg "Тебя спасать, между прочим, выскочат. Вот только ты к концу этого цикла забудешь всё, а вспомнишь уже потом, когда все произойдет."
    ...
    th "И, кажется Лена это знает, если задала такой вопрос. А если еще не знает, то догадается."
    ...
    th "Но уже скоро, чувствую, что от хорошего пинка, она проснется прямо сейчас и проснется безболезненно. Обойдемся без пинков, все должно быть естественно."
    ...
    "Они тоже сидят своей компанией: Семен, Ульяна-большая, Ульяна-маленькая и, чуть поодаль, все три Ольги."
    "Но Ольги уткнулись носом в какие-то вожатские бумаги и, кажется, им не до нас. Когда еще получится встретиться?"
    "Пока все ресурсы системы заняты на то, чтобы выкинуть меня из здешнего мира, любые двойники могут сосуществовать в одном узле и не аннигилировать, но сколько мне здесь еще находиться? Час-два, вряд ли больше."
    hide mt
    hide d_mt
    hide d_mt2
    show d_sf normal pioneer:
    ...
    show us laugh sport
    with dspr
    d_gg "Ну прости. Я больше не буду."
    show d_sf laugh pioneer with dspr 
    me "А куда-ты денешься?"
    show us normal sport
    show d_us normal dress
    with dspr
    d_gg "Есть много миров, кроме вашего. В том числе те, куда еще никто не заглядывал."
    th "Но за вами тоже подглядывать буду, тут я наврал Семену, и мы оба это понимаем, и не только мы."
    show d_sf serious pioneer with dspr
    ...
    show d_us shy dress
    with dspr
    us_old "Не будет он… Ты уже отравился «Совенком». Подглядывай, тебе можно."
    ...
    show us normal sport with dspr
    d_gg "Ульяна, зря ты про миксов переживала. Вон, здешняя Мику проснулась и прекрасно себя чувствует."
    ...
    show d_sf normal pioneer
    with dspr
    d_gg "Все бы ты смогла. Решилась же тогда, в девяносто втором. И другие смогут или уже смогли, им просто нужно вспомнить."
    show d_us normal dress with dspr
    ...
    "Про Мику из их лагеря молчим, оба понимаем, что ее случай из ряда вон выходящий. Что Мику за два дня стала старше на целую жизнь."
    "Сидим еще некоторое время молча. Слышны только общий гул голосов и две гитары."
    ...
    "И столько азарта в их глазах и так хочется дождаться конца состязания, но чувствую, что время уже поджимает, что мне все труднее и труднее удерживаться в лагере."
    d_gg "Я сейчас подойду."
    "Встаю и ищу глазами…"
    hide d_sf
    hide d_us
    with dissolve
    ...
    "Есть еще несколько человек с которыми я бы пообщался, но и время поджимает, и столько общения уже тяжело для меня."
    ...
    with dissolve
    "Коротенькое платьице, сандалики, бантики, две жиденьких светленьких косички. И возмущенный взгляд серых глаз."
    d_gg "Как тебя зовут, большая?"
    ...
    show d_sl happy pioneer with dissolve_fast
    d_gg "Ну, удачи тебе, Глафира Андрейко Денисовна."
    hide d_sl
    hide d_bg
    with dissolve
    ...
    show uv smile far with moveinbottom:
        xcenter 0.89
    "Мы остались втроем на костровой поляне: я, Ульянка и выскочившая из кустов, как только все ушли, Юля."
    ...
    d_gg "[ggname]. Как Мику и написала."
    show us surp3 sport with dissolve_fast
    us "Значит это правда?{w=0.7} Значит и у нее в книге ты не придуманный, а живой!"
    d_gg "Конечно, у меня же вы тоже живые."
    ...
    "Ульянка думает о чем-то своем, я опять верчу головой, чтобы запомнить детали."
    ...
    us "Мы еще увидимся? По настоящему?"
    d_gg "Каким образом, Рыжая белка? У вас я могу существовать только несколько часов и в чужом теле, вот как сейчас; у нас ты — только в виде картинки на мониторе."
    d_gg "Разве что, в следующей жизни. Либо ты к нам, либо я к вам, как получится."
    d_gg "Так что, если в лагерь приедет новенький. Высокий, сутулый и худой, по характеру и любви к книгам и технике, что-то среднее, между Электроником и Женей, присмотрись к нему, прежде чем подбрасывать членистоногих в пюре."
    ...
    d_gg "И да, и нет. В основном деталями."
    d_gg "Например, вот скажи Рыжик, у тебя же веснушки с шеи переходят на плечи и дальше на грудь? По моему — очень мило."
    show us surp3 sport with dspr
    pause 0.7
    ...
    d_gg "Нет, Рыжик, я догадался. Эти веснушки — обычное для рыжих дело, а у нас про них никто не вспомнил. Ладно, прощай."
    show us normal sport with dissolve_fast
    us "Прощай.{w=0.5} Нет, подожди, время еще есть. Побежали, я тебя с Майей познакомлю!"
    ...
    "«Что еще за Майя такая?» — бурчу про себя, но послушно бегу за Улькой по шоссе."
    "Двести метров,{w=0.3} пятьсот,{w=0.3} восемьсот…"
    "Ульянка останавливается в одном ей ведомом месте и ждет меня. Догоняю, оглядываюсь."
    scene dct_ext_outside_field
    show dct_ext_outside_field_Maya
    show us normal sport far at cleft
    with irisout
    "Когда-то здесь был сверток с шоссе на Старый лагерь. Потом лагерь закрыли, а дорожную насыпь срыли бульдозерами."
    ...
    "И еще есть она: девочка, пионерка, как будто из моего старого-старого отряда. Футболка, шорты, галстук на голой шее, стрижка, закрывающая уши. Лет ей двенадцать или тринадцать, не больше."
    ...
    d_gg "Здравствуй, Майя."
    "Протягиваю правую руку и осторожно трогаю ее бронзовое запястье."
    ...
    us "Врешь ты все, что никогда здесь не был! Ты же все сделал правильно!"
    us "А теперь, побежали обратно."
    th "И действительно, пора, а то Семен проснется не в автобусе, а непонятно где."
    ...
    us "Послушай, вот ты наблюдал за нами. А, можно я тоже буду наблюдать, как там ты живешь?"
    d_gg "Конечно можно. Мне будет очень приятно, что ты обо мне беспокоишься."
    show us shy sport close with dspr
    show dct_uv_newdressedging_close at cleft:
        alpha 0.0
        pause 1
        linear 1 alpha 1.0
    "Обнимаюсь с Ульянкой, целую ее в щеку,{w=0.8} обнимаюсь с прибежавшей Юлей, она целует меня в нос и хохочет — отомстила, и лезу в автобус."
    stop ambience
    scene bg int_bus with slideright
    "Все на месте: и пальто, и пакет. Сейчас я усну, а проснется уже Семен, и, через положенное время, выйдет из автобуса и пойдет к воротам «Совенка» навстречу Славе."
    "А я уже, наверное, не стану узнавать, что его там ждет."
    ...
    $ persistent.sprite_time = "day"
    "Все, спать! Посчитаю-ка я для разнообразия автобусы: «Первый четырестадесятый подъехал к остановке, второй четырестадесятый подъехал к остановке, третий...»"
    ...
    show unblink
    d_gg "Повинуюсь, мой злобный хомячок!"
    window auto
    voice "Все написал?"
    d_gg "Возможно…"
    ...
    "Все хорошо, сестренка.{w=1} Я сейчас бросил монетку и надеюсь, что она останется стоять на ребре."
