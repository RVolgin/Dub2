...
label dct_system_errors:
    ...
    with dissolve
    el "Шурик, как же так?"
    "Сыроежкин огорчался совершенно искренне. Столько работы и все зря."
    el "Ты ведь не мог ошибиться."
    show el sad pioneer with dspr
    el "И я за тобой все проверял. И схема работает, сигнал — вот он."
    "Сергей кивнул на колечко светящееся на экране осциллографа."
    
    scene bg int_clubs_male_day
    show sh normal pioneer close at cright
    with dissolve
    sh "Сергей, я думаю, что просто мы еще слишком мало знаем о мозге, и ошибка закралась в самом начале."
    "Шурику было не удобно перед товарищем, но он старался не показывать виду, сказывался опыт Александра, которому случалось представлять неудачи в экспериментах как запланированные результаты."
    
    scene dct_ext_another_clubhouse_day with squares
    "В окно открывался вид на заброшенное здание напротив, логово Яны."
    "Она и сейчас где-то там, внутри.{w} Или еще где-то."
    "Существование Яны напрочь игнорировалось системой, поэтому уверенно видели ее только те, кто знал об ее существовании и уже необратимо проснулся, а таких было, только один Семен и вот, где-то в полушаге к этому состоянию, Ульяна."
    "Да еще Шурик и Мику, ударившие по своим мозгам, сначала модулированным ультразвуком, потом — двадцать пятым кадром. Но Мику про Яну ничего не знала и, поэтому, пока отпадала."
    ...
    sh "\~ Я еще не готов к активации, нужно оценивать объективно. Даже не знаю, должен ли я огорчаться этому. \~"
    show sh normal pioneer with dspr
    sh "\~ Но вот Яну жалко. \~"
    sh "\~ Надо попросить Семена, чтобы приглядывал за ребенком, пока меня не будет."
    ...
    "Что-то с напарником было не так еще со вчерашнего дня.{w} Вот и сейчас, Шурик стоял замерев, уткнувшись лбом в засиженное мухами стекло, и только легкое шевеление при каждом вдохе выдавало в нем живого человека."
    el "\~ Переживает. \~"
    ...
    el "Саша, да черт с ним, с этим прибором."
    show sh normal pioneer with dspr
    show el normal pioneer at center with MoveTransition(1.3, time_warp=_ease_time_warp)
    # "Электроник подошел и встал рядом." # По канону
    el "Зато мы теперь знаем, что вот эта схема не работает и нужно делать другую! Давай подумаем, чем мы можем еще заняться до конца смены."
    ...
    with dspr
    sh "\~ Никогда не высовывавшаяся за пределы «Совенка» и Сети, потому добрая и наивная. \~"
    show sh normal pioneer
    show el serious pioneer
    with dissolve_fast
    sh "\~ Но он еще что-то говорит. \~"
    ...
    sh "\~ Это, правда, убьет Яну-робота и нынешнего Шурика, но кому они нужны, кроме друг-друга? Они так, побочный результат сбоев в экспериментах. \~"
    sh "\~ Но то, что дело не в нашей установке, ты ошибаешься, дело именно в ней. \~"
    show sh normal_smile pioneer with dissolve_fast:
        ycenter 0.5
    sh "Спасибо, Сергей. Дело именно в установке, что бы ты не думал. Я справлюсь, не переживай."
    ...
    sh "Давай разберем этот агрегат и объявим всем, что факир был пьян и фокус не удался."
    ...
    sh "\~ А Александр бы сейчас проворчал: «С кем я связался!? С миксами! — Или так. — Дожили! Два влюбленных микса!». \~"
    ...
    th "А вот сейчас, взял лодку, приплыл следом за мной на остров, перешел на другую сторону, встал у самого берега и, чуть повернув голову, чтобы ветер был в лицо, замер. Наверное и глаза закрыл."
    ...
    "Максим смутился."
    "Вот только что еще, по инерции смотрел куда-то сквозь Лену за горизонт и улыбался чему-то своему, а сейчас говорит опустив глаза и чуть-чуть покраснев."
    d_ma "Понимаешь, Лена, я сейчас вышел из столовой, встал на крыльце и вдруг понял, насколько он маленький, наш лагерь. А я даже на территории еще побывал не везде."
    "Максим еще помялся и выдохнул."
    ...
    un "Ну, неделю еще поживешь тут..."
    th "Семен будет доволен."
    show d_ma smile pioneer with dspr
    th "Максиму еще расти и расти, но, кажется ему есть куда расти, кажется, когда проснется, он в лагере, на одном месте, сидеть не будет."
    un "… а потом закончится смена и делай что хочешь."
    show d_ma rage pioneer with dissolve_fast
    d_ma "Да какое там, что хочешь… Школа, родители и все решают за тебя."
    ...
    d_ma "А оказалось, что не соврал. Оказалось, что это легко, когда поймешь как."
    ...
    with dissolve
    "Легкий ветерок шевелил волосы, слышно было, как шлепают волны о берег."
    "Лена наблюдала за охотящейся скопой: как скопа парит над водой, на уровне вершин сосен, как она, увидев рыбу, пикирует и входит в воду, выставив перед собой ноги, как появляется на поверхности и, сделав сильный взмах, тяжело отрывается от воды с рыбой в когтях, и несет рыбу куда-то на остров Длинный."
    ...
    th "А ведь Семен его вместо себя оставить хочет. Сам еще не знает об этом, но уже хочет."
    extend " Подтянет его, оставит вместо себя, а сам уйдет."
    th "Даже не важно, физруком будет Максим или кем-то еще, просто кем-то, на кого можно оставить лагерь, потому что вожатая не всесильна."
    th "А сам уйдет и Ульяну с собой уведет."
    extend " Жалко, очень жалко."
    show d_ma blink pioneer with dspr
    th "Но Алисе будет к кому возвращаться."
    th "А позже и Максим уйдет. И Алиска уйдет, рано или поздно, вон, вместе с Максимом и уйдет."
    th "И я уйду, когда мой Семен готов будет, это еще не скоро, но я подожду."
    th "Потому что нельзя вечно жить в детском манеже."
    extend " Будем заглядывать иногда, невидимые пионерами. Да, если между сменами попадем, будем с Ольгой общаться."
    ...
    th "А вот куда уйдем, я не знаю. В Шлюз? В пустые узлы? В материнские миры?"
    th "Ульяна говорила, что выход туда через теплообменник скоро закроется, но ведь есть же еще точки перехода в пещерах."
    ...
    extend " А наш еще — теленок-теленком, хотя, в спектакле сыграл неплохо."
    ...
    us "Не, я еще не нагулялась.{w} Вот, держи, принесла."
    "Лена взяла протянутые ей листки с текстом, листки, с ее собственными рисунками и с какими-то пометками на обратной стороне."
    ...
    us "Вот только к сестренке забегу еще.{w=0.5} Этого будить?"
    ...
    extend " И при этом порхала по библиотеке как мотылек."
    show d_sf smile pioneer with dissolve:
        xcenter 0.5
    me "Женя, а правда, что влюбленные глупеют?"
    show d_mz shyangry glasses pioneer far at fleft with dissolve
    mz "Ты это к чему?"
    "Женя-новая мгновенно превратилась в Женю-прежнюю, но, заметив улыбку Семена, отыграла назад."
    show d_mz smile2 glasses pioneer far with dspr
    mz "Да, случается."
    show d_mz smile glasses pioneer far with dissolve_fast
    extend " А что, заметно?"
    mz "\~ Где та Евгения, которая орала на Сережу в автобусе в день приезда? \~"
    show d_sf sad pioneer
    show d_mz shy glasses pioneer far
    with dspr
    "Женя мельком глянула на Семена, склонившегося над журнальным столиком и что-то пишущего, и подмигнула своему отражению."
    ...
    "Скрипнула дверь и в библиотеку зашла, опасливо косясь на Женю, одна из мелких, прошептала: «Здрасьте», — подбежала, прошлепав по половицам босыми ногами, к Семену и что-то спросила у него на ухо."
    show d_sf smile pioneer with dspr
    "Семен улыбнулся, достал из кармана ключи, отцепил один от связки."
    hide mz
    hide d_mz
    with dissolve
    ...
    "Мелкая улыбнулась в ответ, часто-часто закивала, качнулась опять к Семену, будто хотела то ли еще о чем то спросить, то ли просто прижаться, но застеснялась и передумала."
    ...
    "Семен проводил девочку взглядом, кивнул своим мыслям, и опять уткнулся в бумагу."
    stop ambience fadeout 1
    show black with dissolve
    ...
    "Зачем это нужно — непонятно, все равно придут те же полтора читателя, которые на стенд даже не посмотрят. Но вот, должен быть оформлен стенд, значит его нужно оформить."
    ...
    "Поэтому, когда Семен отчетливо пробормотал, комментируя что-то в собственных записях: «Ах, закройте, закройте глаза газет!» Женя вздрогнула:"
    ...
    mz "Конечно. Мы, оказывается, живем в соседних городах, пять часов на поезде."
    mz "Я приеду к нему в гости на каникулы, а на следующий год Сережа приедет поступать в наш университет.{w} Он хотел в Бауманку, но передумал и решил в наш университет, на мехмат…"
    ...
    mz "Я не обманываю себя, Семен. И не думаю, что наши отношения продлятся намного дольше, чем эта смена."
    ...
    mz "Так получилось, что первой девушкой, которую увидел Сережа, подняв голову от паяльника, оказалась Жужелица."
    ...
    "Но в Жужелицу, в которую были влюбленны, как минимум две недели, а это — огромная разница с прежней."
    show d_sf serious pioneer with dspr
    "Семен посмотрел очень серьезно, чуть пожал плечами и наклонил голову, как бы не со всем соглашаясь."
    me "Ну, кто же тебя в насекомых держит?"
    show d_mz normal glasses pioneer far with dspr
    me "На общественное мнение тебе плевать, Сыроежкин в тебе дыру вот-вот проглядит, значит остаешься только ты сама."
    show d_sf normal pioneer with dspr
    me "Я всегда считал, что это дело двоих. И ответственность, и право двоих — решать сколько им быть вместе.{w} Даже если один из них внезапно убегает, решение об этом всегда принимают вдвоем, может не замечают этого, но вдвоем."
    show d_sf sad pioneer with dspr
    "И замолчал. А через три минуты, перечитав еще раз написанное, сложил листки бумаги пополам, спросил у Жени, есть ли у той в хозяйстве конверты, получил конверт из оберточной бумаги, спрятал в него написанное, собрался заклеить, но передумал."
    "И, написав что-то на конверте, но явно не адрес, сунул его в карман, попрощался и вышел бросив напоследок:"
    show d_sf smile pioneer with dspr
    "Пойду спасать спорткомплекс от малолетних варваров. Улька добрая, она мелких не тронет, она меня схарчит."
    hide d_sf with dissolve
    show d_mz sceptic glasses pioneer far with dspr
    th "Интересно, что такого физрук писал?"
    show d_mz surprise glasses pioneer far with dspr
    "А потом Женя вспомнила, что в пять часов придет Сережа, а у нее еще чайник не поставлен, и отбросила этот вопрос, как несущественный."
    ...
    show promo_text_un u"{color=#e1dd7d}Из записей Семена Персунова{/color}" at truecenter with dspr
    $ renpy.pause(3.0)
    ...
    window auto
    el "Привет, Мику."
    mi "Здравствуй, Сережа. Ты из клуба?"
    ...
    el "Шурик так расстроился, что взял и стер всю программу дешифровки.{w} И машину мы размонтировали."
    show mi upset pioneer far with dspr
    mi "Сережа, а может и правильно, что размонтировали? Представь, вдруг мы бы узнали о себе что-то такое, что… Ну, лишило бы нас радости."
    ...
    "Стоило той отвлечься, как Витька, вместо уставной отрядной речевки-кричалки, выдавал:"
    ...
    d_guys "Раз, два! Левой, правой! Мы идем топить котят!"
    hide d_vi with dissolve
    "Понятно, что никаких котят никто топить и не собирался, если бы в лагере оказался хоть один котенок, его бы скорее зацеловали, загладили, закормили и затискали. А за намек на «утопить», намекнувший сам оказался бы на дне."
    ...
    el "Если это какое-то событие, оно уже в прошлом, а мы то живем сейчас.{w} Если это какой-то наш проступок, то, я не знаю, надо попросить прощения за него и заслужить, чтобы тебя простили."
    show mi sad pioneer with dissolve_fast
    mi "А если ничего не исправить уже?"
    show el angry pioneer with dissolve_fast
    el "А тогда остается только двигаться дальше, а не стонать и ныть. Ты узнала о себе что-то новое, значит пользуйся этим. Я бы так поступал."
    ...
    el "Да что говорить: установку разобрали, программу стерли, — теперь никто ничего ненужного не вспомнит."
    show el normal pioneer
    show mi sad pioneer
    with dissolve_fast
    ...
    $ persistent.sprite_time = "sunset"
    "\~ В этом лагере что-то можно долго хранить в секрете? \~ — пришла ему в голову запоздалая мысль, но он отмахнулся от нее, как от несущественной."
    ...
    # "Семен и Ульяна, как и позавчера, сидели на крыльце спорткомплекса, смотрели на звезды, на блестевшую за аллеей и пляжем реку, на темную массу острова Длинный, закрывающую горизонт."  # По канону
    window auto
    "Семен и Ульяна, как и позавчера, сидели на крыльце спорткомплекса, смотрели на звезды, на блестевшую за аллеей и пляжем реку, на железнодорожный мост раскинувшийся от края до края бухты."
    "Было тихо, до отбоя оставалось еще около часа, но пляж уже опустел. Прохладный ветерок с реки забирался Ульяне под футболку, и девушка зябко вжималась в теплый Семенов бок."
    me "Может, внутрь зайдешь?"
    "Но у Ульяны была другая идея."
    us "Не, Сём, я сейчас ветровку накину и мы пойдем погуляем.{w} Тебе, кстати, тоже захвачу, а то с твоей спиной..."
    ...
    "Глянула на Семена, смотрящего в небо, лукаво улыбнулась и стерла надпись. Только одна буква «У» и осталась видна."
    "Не сговариваясь физруки повернули вдоль берега по заросшему березами и кустарником участку между пляжем и пристанью, вдоль невысокого, по колено, обрывчика."
    ...
    play sound_loop "<to 23.0>mods/dublicate_completed/sounds/sfx/dct_sfx_cry.ogg"  # Плач, но обрезанный, а то он там прям в рыдания переходит
    ...
    th "Это хорошо что сама, потому что я не силен в любовной тригонометрии."
    stop sound_loop fadeout 4
    show d_sf sad hike:
        pause 1.5
        linear 1 alpha 0
    "Ульяны не было довольно долго, Семен прошел по мосткам и спрыгнул в ближайшую лодку. Развалился там, вытянув ноги на кормовой банке, и прикрыл глаза."
    "Спать не хотелось, хотелось слушать вечернюю тишину: плеск воды о дебаркадер, поскрипывание мостков, перестук бортов лодок, тихие девичьи голоса, шелест листвы близких берез."
    "Голоса смолкли и послышалось шлепанье по палубе дебаркадера двух пар босых ног."
    stop sound_loop
    ...
    me "Почему нет? Пойдем, Кать. Ты до домика с нами?"
    ...
    with dissolve
    "Семен выбрался из лодки, поддержал девушек, пока они обувались, и необычная компания продолжила обход лагеря."
    ...
    me "\~ Ничего, через восемь дней все забудешь, а там Максим окончательно пропишется в старшем отряде и для тебя, считай что просто исчезнет. \~"
    ...
    me "Узнаешь еще, какие твои годы."
    ...
    show d_sf serious hike with dspr
    th "Что-то там переключает Система в их поведении. Гадко это, но, чтобы проснуться, им наверное придется пройти и через это."
    "Ульяна и Катя говорили о чем-то, а Семен все думал.{w} Как раз то обстоятельство, за которые Ульяна и обзывала его тормозом."
    show d_ka smile pioneer with dspr
    ...
    "И, дождавшись кивка от Семена, Катя забежала на крыльцо своего домика. Помахала рукой с крыльца и скрылась за дверью."
    show d_sf normal hike
    show d_us serious sport_windbreaker:
        xzoom -1.0 xcenter 0.55
    with dissolve_fast
    us "Сёмк, ей всего то и нужно было, чтобы кто-то с ней поговорил и ее выслушал.{w} А у них в отряде некому, раз уж она там…"
    ...
    with dissolve
    "Яна заметила Персуновых, что-то сказала Шурику и помахала им рукой. Шурик солидно кивнул Семену и Ульяне, достал из кармана ключи от кружка."
    sh "Зайдете?"
    me "Нет, мы еще погуляем, спокойной ночи."
    hide d_jn
    hide 3500_sh
    with dissolve
    me "Я не знаю, что там вспомнил Шурик, но он нашел для себя якорь."
    ...
    me "Не знаю, Рыжик. Но теперь ему деваться некуда. Сейчас он проснулся или проснется через сто циклов, какая разница."
    me "Куда дальше идем? На площадь или к Мику?"
