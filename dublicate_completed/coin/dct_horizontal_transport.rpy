...
label dct_horizontal_transport:
    ...
    with dspr
    "Семён начал листать:{w} Женя с разными выражениями лица;{w} Шурик, поправляющий очки;{w} Максим с горном, сидящий на скамье;{w} тот же Максим, спящий в тени березы;{w} две Ульяны, большая и маленькая, хлопающие друг-друга пятернями;{w} Ольга Дмитриевна, читающая нотацию среднему отряду."
    ...
    "Лена посмотрела на Семена неожиданно доверчиво. И, как когда-то рассказала ему о себе и Семёне-втором, начала свой рассказ."
    show anim prolog_1:
        alpha 0
        linear 0.5 alpha 1
    $ set_mode_nvl()
    ...
    "А потом, Семён унес Ленину иллюстрацию в лагерь Виолы, а через два часа перед Леной появилась недовольная, тем что её оторвали от каких-то своих дел, Ульяна-маленькая и передала новую работу Мику из того лагеря, и её просьбу: «Что-нибудь с этим сделать»."
    ...
    sh "Счастье твоё, Яна, что сегодня воскресение и можно спать до девяти утра."
    ...
    sh "Дочка, ты задаешь такой вопрос, на который отвечает, даже для самих себя, едва ли десятая часть всех людей.{w} И то, многие только в конце жизни. А большинство живёт не думая, просто как трава растёт."
    ...
    with dissolve
    "Ульяна удалилась куда-то в сторону хозяйственных ворот.{w} Саша вздохнула. Чувствовалось, что Ульяна всеми силами старается поддерживать от неё дистанцию, а причина была не понятна."
    ...
    $ persistent.sprite_time = "day"
    "А всё потому, что вчера вечером Семён изрек:"
    ...
    show un shy pioneer with dspr
    "За что был вознагражден приглушённым, но восторженным воплем Ульяны и её репликой:"
