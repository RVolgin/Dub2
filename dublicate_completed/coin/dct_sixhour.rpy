...
label dct_sixhour:
    ...
    with dissolve
    "Сыроежкин спит на соседнем кресле.{w=0.4} Сейчас он проснётся и начнет знакомиться."
    ...
    th "Господи, ну когда же я сдохну?{w} Когда мой сосед по этому телу разовьется настолько, что захватит над ним контроль целиком? Как же мне надоело просыпаться."
    ...
    show dct_int_bus_another_middle_mz_head upset
    pause 1.0
    "По проходу быстрым шагом прошёл Семён. Да, Александр узнал его, вот только, много циклов назад, перед расставанием в посёлке, Семен выглядел на семнадцать лет, а сейчас казался двадцатипятилетним."
    ...
    "Александр встал, извинился перед Сыроежкиным и, не глядя ни на кого, подошел к Семёну."
    ...
    show dct_int_bus_people_vi sitdown
    with dissolve
    "Повеселевший Семен, сначала попросил весь средний отряд собраться в одном автобусе, чтобы не повторять два раза. Потом, когда просьба была выполнена и все пионеры собрались в Икарусе, от администрации лагеря последовала ещё одна вводная."
    ...
    adl_sh4 "Все равно, с обходными бегать. Ты иди сейчас, если хочешь. Заодно посмотришь, что там есть. А я... поработаю."
    ...
    $ persistent.sprite_time = "night"
    th "Ну почему, почему у меня всегда вс` наперекосяк выходит?"
    stop music fadeout 2
    ...
    us "Мы скоро уйдем. Попрощаемся с тобой, и уйдём. А то нехорошо тебя просто так отпускать."
    show d_sf normal pioneer with dspr
    ...
    play ambience ambience_int_cabin_evening fadein 1.5
    hide black with dissolve
    "Когда пришел Семён, Ульяна сидела в бывшей тренерской, а теперь официальной квартире Персуновых, и всхлипывала."
    ...
    show dct_clubs_male_comp_table:
        xcenter 0.5 ycenter 0.45
    with dissolve
    "Надел наушники, сел перед экраном, стёр содержимое одной из ячеек таблицы, заменив его на ещё одну бессмысленную цепочку из символов, ввел пароль, и, как полчаса назад, нажал «Ввод»."
    play sound_loop sfx_computer_noise fadein 0.5
    ...
    th "Надо же, заработался. И голова болит нехорошо, не пришлось бы завтра в медпункт обращаться. Планов много, а смена такая короткая. Надо выспаться, и все пройдёт"
    ...
    "Она мало походила на человека, половина её алюминиевого лица была закрыта чёрным, зеркальным щитком, а вместо носа и рта чернела совмещенная решётка газоанализатора и динамика."
    show d_jn:
