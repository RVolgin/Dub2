...
label dct_transitions:
    ...
    "Электроник проснулся в тот момент, когда мимо их домика в сторону стадиона пробежала Саша. Полежал еще, прислушиваясь к удаляющимся шагам, покосился на спокойно спящего соседа."
    th "Хорошо, что вчера Шурик в кружке ночевать не остался"
    th "Надо будет все же над ним шефство взять, а то он легко может загнать сам себя. Такие гении нужны человечеству."
    ...
    "Электроник улыбнулся, вспоминая тяжесть Жениной головы у себя на плече и запах ее волос."
    ...
    "Поколебался пару минут и решил все-таки разбудить. Сегодня же первый день смены, после завтрака будет первая линейка, важные объявления и прочее. Не стоит пропускать."
    ...
    "Желто-бурый песок, серо-желтые редкие кустики, какое-то белесое небо и неожиданно холодный и резкий, продувающий насквозь пальто, свитер, тело под пальто и свитером, и летящий дальше ветер."
    ...
    show dct_int_house_of_el_day:
        ...
        pause 1
        linear 1 blur 0
    "Шурик огляделся вокруг, нашарил на стуле очки и огляделся еще раз."
    "Небольшой домик: комната на две кровати и прихожая, отделенная от спальной зоны двумя шкафами."
    "На кровати напротив сидит Сыроежкин, и пытается сидя заправить в шорты рубашку."
    th "Что за странный сон?"
    th "Там все выглядело так, будто и те люди, и то место мне давно знакомы."
    th "Может, сон о будущем? Теоретически, какая-то информация из будущего может просачиваться в наше время."
    th "А Сергей был в пионерской форме, потому что я еще не представляю, каким он будет."
    ...
    mi "Женечка, ты слышишь, слышишь? Горн. Настоящий, а не запись! Вчера ко мне подошел один мальчик и попросил послушать его, как он играет."
    ...
    show mi serious pioneer far:
        ...
        pause 0.5
        linear 2.5 alpha 0
    "Женя только вздохнула. Силы рычать и ругаться на соседку закончились еще вчера."
    ...
    show mi upset pioneer far:
        alpha 0
        linear 1 alpha 1
    mi "… а здесь играет только Алиса, я видела у нее гитару. И еще в кружке, в журнале выдачи, записано, что одна гитара на руках у физрука."
    ...
    th "Хоть бы помог кто. Подвел бы его ко мне и сказал: „Женя, познакомься, это… Сергей“. Тогда я бы, может, так не стеснялась."
    show mi scared pioneer far:
        alpha 0
        linear 1 alpha 1
    mi "… а еще этот, Сыроежкин Сережа. Он ведь тоже играть умеет, на ксилофоне. У меня в кружке даже ксилофон есть, но Сережа отказался, почему-то, даже показалось, что убежал от меня. Неужели я такая страшная?.."
    show mi scared pioneer far:
        alpha 1
        pause 0.5
        linear 2.5 alpha 0
    th "А мне вот убежать некуда."
    th "Вот должна была Саша со мной жить, А Мику — с Леной, но Лена наверное в автобусе еще с Сашей договорилась, и они с Мику поменялись."
    th "А я согласилась, я еще не знала, что меня ждет."
    ...
    mi "… Женечка, подожди, я тоже еще не умывалась. А тут двое октябрят подходили, спрашивали про кружок, но я совсем не знаю, как с маленькими обходиться. Может быть, у тебя в библиотеке есть какие-нибудь методички? Я бы записалась…"
    th "Господи! Нет!"
    stop ambience fadeout 1
    ...
    mt "Хоть вы-то меня, надеюсь, поймете."
    ...
    mt "Это временно, Семен. К линейке пройдет. А пока нас никто не слышит, у меня получается."
    show d_mt normal pioneer close orika as mt with dspr
    mt "Надо кого-то в домик к Алисе подселять, Ульяна уже вряд ли назад вернется."
    ...
    show d_us normal sport
    with dspr
    mt "В общем, вы подумайте. И я подумаю. И даже ментор, который сейчас проснется, тоже подумает."
    ...
    mt "Семен, Ульяна, особенно Семен. Надеюсь, смена обойдется без ваших импровизаций. А то я не могу ни одну смену согласно Плану мероприятий провести."
    ...
    show d_sf smile sport with dspr
    me "Надоело до чертиков, я же ленивый. Знал бы, что так будет, может, и не побежал бы никуда."
    ...
    "И с каждым циклом, начатый вместо убежавшей Яны и частично даже функционирующий робот становился все менее и менее близок к завершению."
    "Шурик проверял на нем какие-то свои идеи, часть деталей перекочевывали, с подачи Алисы, из робота в музыкальную аппаратуру или дискотечный усилитель."
    ...
    "Как же еще быть, если тебя посетила Идея? Конечно же, надо спрятаться ото всех, чтобы тебе никто не мешал и все обдумать."
    show d_el smile2 pioneer close as el with dspr
    "И еще, на линейке будет Женя."
    "Из-за неё-то Электроник и пришел на площадь сильно заранее, когда там еще никого не было"
    show d_sf normal sport:
    ...
    show d_us normal sport:
        ...
        pause 0.7
        linear 0.5 alpha 1.0
    extend ", кроме обоих физруков (Ульяна хоть и числилась начальником спорткомплекса, но все ее в лагере звали либо по имени, либо физруком, как и Семена)"
    ...
    show el serious pioneer close with dissolve_fast:
        xcenter 0.17
    th "Ну правильно, Женя девушка серьезная, что ей в этой хохочущей компании делать?"
    ...
    dv "Лет тебе не меньше, чем было Ульяне, так что все нормально."
    ...
    show d_sf smile sport with dspr
    me "Сам понимаешь, соседство с Алисой тебя до добра не доведет. Она тебя плохому научит."
    ...
    dv "Сенька, придушу!"
    "Алиса потянулась руками к Семеновой шее."
    show d_us smile sport
    show d_ma smile
    ...
    with dspr
    dv "Имей в виду, Сенька он только для меня. И еще два человека могут так его назвать без риска для здоровья и жизни."
    show d_ma serious
    ...
    "А Женя всю линейку поглядывала в сторону нестриженого затылка Электроника и злилась:{w} на себя за то, что так и не решилась извиниться за скандал в автобусе;{w} на Электроника за то, что тот сам до сих пор не подошел к ней;{w} на окружающих, за то что никто еще не догадался познакомить её с кибернетиком."
    mz "\~ Да хоть бы просто поговорить подошел. Хоть спросил бы о том, когда библиотека открывается? \~"
    scene bg ext_square_day with blinds
    "Но все когда-то заканчивается. Закончилась и сверхдлинная первая официальная линейка смены."
    "Пионеры получили обходные листы и были отправлены за автографами, отсутствие Шурика на линейке прошло без последствий, Ольга Дмитриевна только кивнула, когда Электроник попросил у нее два бланка обходных, и вручила требуемое."
    "Электроник просмотрел обходные, поискал глазами на площади — может, еще не ушли те, чьи автографы он должен собрать."
    show d_sf normal sport:
    ...
    show sl smile pioneer far:
        xcenter 0.88
    with dissolve
    th "Ага, Семен здесь, Мику здесь, за Шурика я и сам распишусь, осталось только в библиотеку и к доктору зайти."
    ...
    hide sl
    with dissolve
    "Он еще услышал, как Мику воскликнула: «Отлично, значит в восемь вечера у меня в кружке!» — и пошел собирать автографы за себя и за Шурика."
    stop ambience fadeout 1
    ...
    "Вообще-то, в библиотеке должно быть тихо, а в библиотеке небольшого пионерского лагеря еще и безлюдно."
    ...
    "И все они решили собрать эти подписи еще до обеда, чтобы после обеда бездельничать с чистой совестью."
    scene bg int_library_day:
    ...
    "И еще нужно был ставить эти чертовы подписи."
    hide d_mz with dissolve
    "Женя, из-за этой суматохи, даже о Сыроежкине почти не вспоминала, пока тот не пришел, уже перед самым обедом."
    ...
    mz "— Ну кого еще с-силы небесные принесли?!"
    "Никто не ответил, но кто-то все же зашел."
    ...
    "Женя приготовила на лице неприязненное выражение (само приготовилось, чего уж там, не владела Женя актерским мастерством), набрала воздуха, чтобы высказать пришельцу все что думает («Говори четко! Проси мало! Уходи быстро!») и замерла, не зная, как реагировать."
    scene bg int_library_day
    ...
    "Потом мысли ее переключились на Электроника:"
    ...
    "Некоторое время боролась с торчащим вихром, потом обреченно махнула на него рукой."
    show d_mz sceptic glasses pioneer with dissolve_fast
    "Расправила плечи и распрямилась, посмотрела на своё отражение."
    hide d_mz with dissolve
    "Встала в профиль и крутанулась на пятках. Несколько раз прошла туда-сюда по комнате, оглядываясь на зеркало. Потом представила, как все это проделывают те же Мику или Саша, представила себя рядом с ними — уродина."
    ...
    mz "Ну, значит, и вам никто не нужен. Значит, мы с вами обойдемся!"
    ...
    th "Придет — отдам."
    "Потом взгляд ее упал на половую тряпку."
    th "Придется мыть пол вместо обеда."
    th "А то после обеда Сергей придет, а у меня грязно как в подземном переходе. Если придет после всего."
    ...
    show black
    hide black with dissolve
    "Ольга Дмитриевна и Семен обедали за одним столиком."
    ...
    show dct_int_dining_hall_table_day_front
    with dissolve
    mt "Семен, что ты думаешь о Максиме? О том, чтобы перевести его в старший отряд?"
    ...
    mt "И вот еще что…{w} Хотя нет, это я сама."
    show mt normal pioneer close:
        linear 0.5 ycenter 0.23
        pause 0.15
    ...
    "А Семен отнес посуду на мойку, покрутил головой и, не обнаружив Ульяны, пошел к себе."
    stop ambience fadeout 1
    ...
    with blinds
    "В итоге, когда после первой в смене тренировки, пятеро мелких: Вася, Гриша, Оксана и Сергей Зайцевы и Геля, устроившись на футбольной трибуне, выбрали Васю, Семен огорчился."
    "Он ждал именно этого результата, но все равно огорчился. Вот так — прощай, Вася."
    "Через неделю вожатая объявит о переводе, еще неделю ребята отгуляют в своих отрядах, а на следующий цикл уже приедут в новом качестве."
    "И неспящий сейчас Васька будет неизвестно сколько жить двухнедельными циклами, пока не перейдет в старший отряд, и еще неизвестно сколько в нем, пока не проснется."
    show d_va upset:
    ...
    "Семен мысленно поздравил Максима, у которого появились шансы, и грустно посмотрел на Ваську.{w} И на Оксану, взявшую Васю за руку, тоже."
    th "Их обоих бы передвинуть"
    extend ", нет — троих, еще и брата ее."
    ...
    "Октябрята отошли на несколько шагов и о чем-то разговаривали вполголоса."
    show d_va serious:
    ...
    d_va "У меня еще две недели здесь."
    d_va "Я пока буду прощаться, а потом, Семен, до конца смены, поможешь мне с лодкой?"
    show d_va sad with dspr
    d_va " Я сам не успею, а очень хочу пройти под парусом."
    me "Обязательно. Как Второй приедет и все устаканится, так и сделаем."
    "Семен неумело и неловко прижал Ваську за плечо к себе, а Васька, также стесняясь, ткнулся головой ему в бок."
    show d_va shy with dspr
    me "Не жалеешь, что уходишь?"
    show d_va sad with dspr
    d_va "Жалею.{w=0.8} Но, Семен.{w=0.5} Мы никогда об этом тебе не говорили, но мы все очень-очень хотим вырасти."
    d_va "А вырасти можно только так."
    d_va "И все ребята еще утром сказали, что я должен вырасти первым."
    stop ambience fadeout 1
    show black with dissolve
    ...
    hide black with dissolve
    "Наконец, все что требовалось было найдено и отложено. Невостребованные детали перекочевали обратно в ящик, который занял свое прежнее место под одним из верстаков."
    show sh normal pioneer close with dspr
    "Шурик еще раз сверился со схемой, кивнул сам себе, посмотрел как травится плата, посмотрел на часы."
    show sh serious pioneer close with dspr
    th "Плохо, что нет ни азотки, ни хлорного железа. В медном купоросе плата травиться будет еще часа три-четыре."
    stop sound_loop fadeout 1
    ...
    show mt normal pioneer close with dspr
    mt "… В общем, задание вашему кружку. Сделать так, чтобы, начиная с завтрашнего утра, сигналы горниста были слышны изо всех динамиков по всему лагерю. Усилитель вы знаете где, что еще нужно спаять — разберетесь."
    ...
    el "А Женя, она такая строгая, я не хочу, чтобы она ругалась потом, если я не приду. И вожатой обходные без ее подписи не отдашь."
    ...
    "Вздохнула, глянув на пустующую Ульянкину кровать, махнула на нее рукой, садись мол. А сама полезла искать что-то по шкафам и тумбочкам."
    show d_ma normal pioneer:
    ...
    dv "А то к духовым инструментам, кроме нее здесь никто и никак. В общем, сделаешь так, как она скажет."
    dv "И третье, вожатая согласна на твой перевод в старший отряд. Через неделю на линейке объявит, если ничего не изменится."
    dv "Но не думай, что у тебя какие-то права появятся после этого. Обязанности уже появились, а права еще нет."
    show d_ma surprise pioneer with dissolve_fast
    dv "Следующую неделю будешь еще числиться в среднем отряде, а в старшем — только кандидатом."
    ...
    show d_ma normal pioneer with dissolve_fast
    "Алиса ждала, глядя насмешливо и надменно, а соломенноволосый и голубоглазый Макс (Отрок Варфоломей, как его однажды назвала еще баба Глаша), обвел взглядом интерьер домика Алисы, потрогал пальцем шину на велосипедном колесе и спросил, невинно глядя ей в глаза:"
    d_ma "Тётя Алиса, а когда ты начнешь меня учить плохому?"
    ...
    "Электроник тропинкой шел от библиотеки, почему-то в сторону стадиона и ругал сам себя."
    ...
    "Через час, когда разгоряченные присели передохнуть, один из пионеров достал бутылку воды и пустил ее по кругу."
    show d_el normal shorts at center with dissolve
    pi4 "Серега, ты же хорошо играешь. Охота тебе в своем кружке лето просиживать? Может, свою команду организуем?"
    show d_el smile shorts with dspr
    pause 0.6
    show d_el laugh shorts with dissolve_fast
    "Электроник оглянулся, прежде чем ответить, и неожиданно уперся взглядом в Женю."
    ...
    show black
    hide black with dissolve
    "Семен подошел к тренерской и, услышав из-за двери голоса, задержал руку на дверной ручке."
    th "Ульяна и кто?"
    ...
    show sh surprise pioneer close at cright:
        ycenter 0.70
    with dissolve
    "Когда, незаметно для себя, Шурик начал задремывать, вдруг показалось, что прямо над головой, по чердаку, пробежал кто-то легкий."
    show sh normal pioneer close with dspr
    ...
    "Это завораживало настолько, что обеим захотелось выйти и присоединиться. Если бы они умели еще при этом."
    us "Очень красиво."
    ...
    "Ульяна-большая представила себе «сестренку» и, не задумываясь о последствиях, закрыла глаза и шагнула следом."
    ...
    show dct_ext_musclub_backside_night_ma_mi:
        linear 3.5 alpha 0
    "Вспомнила еще, как показалось на секунду, что в момент кувырка младшей на нее из кустов, с противоположной стороны поляны, глянули как-будто два желто-зеленых глаза, принадлежащих очень большой кошке."
    show dct_ext_musclub_backside_night behind d_us:
        alpha 0
        linear 1.5 alpha 1
    th "Всё, сестренку проводила, пора спать. Или, может, посидим еще с Сёмкой. Посмотрим."
    show d_us normal sport with dspr
    show d_us normal sport:
