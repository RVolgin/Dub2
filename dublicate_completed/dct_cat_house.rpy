...
label dct_the_first_life:
    ...
    mic "А ты только и рада будешь, если выгоню. Эта каморка даже для кошки тесная. Но потерпи, командировка закончится, мы на материк вернемся, в нормальную квартиру."
    ...
    mic "Ладно, если просто сбежала. Светка покричит, подуется и простит. А если её нашел кто? Это же ЧП и нарушение режима: домашнее животное на территории посёлка."
    ...
    mic "Свет, она же по сути котенок ещё, дети всегда меняются."
...
label dct_creature:
    ...
    hide cat_uv
    "Кошка шагнула на кухню и прикрыла за собой дверь. Постояла прислушиваясь, запрыгнула на подоконник, с подоконника — вертикально вверх к форточке, а там, зацепившись тремя лапами за переплёт, ухватилась четвертой за форточную завёртку."
    ...
    show cat_uv walking2 medium with dspr:
        pos (0.25, 0.7)
    "Незадолго до рассвета, кошка тем же путем вернулась домой, прихватив по пути ещё одну мышку."
    stop ambience
    ...
    "И каждый из рассказчиков клянётся, что «сам видел». Сам видел горящие глаза, клыки, когти, женскую фигуру, без одежды, но обросшую шерстью. Бывает. Ах да, еще и жуткий вой «сам слышал»."
    ...
    $ persistent.sprite_time = "day"
    play ambience ambience_camp_center_day
    "Так рядового и нашли утром: лежащего в позе эмбриона на полу запертой изнутри будки (пришлось вынимать стекло), поседевшего, и повторявшего все время: «Докладывает дневальный по КПП: на посёлок напал оборотень»."
...
label the_felinoid:
    ...
    $ persistent.sprite_time = "day"
    play ambience ambience_forest_day
    "Охотник прицелился, чуть подавшись туловищем назад, выдохнул, задержал дыхание, и плавно потянул спусковой крючок. Довел его до середины и {w=2} опустил оружие."
    ...
    play ambience ambience_int_cabin_day
    show d_to normal casual at center with dspr
    d_to "Вот, Женя, это твоё рабочее место. Приказы, картотека, личные дела сотрудников, график отпусков. И так далее, и тому подобнее. Разберёшься?"
    hide d_to with moveoutright
...
label dct_catgirl:
    ...
    $ persistent.sprite_time = "night"
    play ambience ambience_camp_center_night
    "Всё также держась поблизости от посёлка, бывшая кошка открыла для себя незаселённые узлы. Где можно было пройтись по абсолютно пустому лагерю, в котором никогда не звучали детские голоса."
    stop ambience
...
label dct_goddess:
    ...
    "Но и это ещё не все. Говорят, что среди наших пионеров-НБО иногда появляются копии людей оттуда, и кончается это трагедией. Либо резнёй в узле, либо узел просто исчезает."
    ...
    "Потом он склонился над алтарем, над каменными желобами, уходящими под основание пирамиды, над бронзовыми кольцами, вделанными в камни."
    ...
    $ persistent.sprite_time = "day"
    play ambience ambience_camp_entrance_day
    "А когда автобус скрылся за восточными воротами; Генда на мгновение превратился в чёрный цилиндр, достающий до неба; а всех обитателей узла, где бы они не были, пробрал секундный озноб. Тогда ей стало так плохо, что она, забыв обещания, шагнула к чёрной пирамиде и, не задумываясь, приложила руки к углублениям в базальте."
    show gate_ext_pyramid:
        alpha 0
        ease 3 alpha 0.5
    $ renpy.pause (1)
    ...
    "Отстегнул. Забросил ружье на твёрдую землю, по соседству с полевой сумкой. Ремешок от полевой сумки и поясной ремень, уже связанные между собой, были намотаны на левую руку охотника."
    ...
    scene dct_to_in_bog with vpunch
    play sound dct_sfx_jump_in_swamp
    d_to "Толяныч, лежи и не дергайся! А то погружаешься. Отдышись и делай ещё попытку."
    scene dct_to_in_bog with vpunch
    play sound dct_sfx_jump_in_swamp
    ...
    "Схватила ружьё, положила его стволами поперек сосенок-близнецов, обмотала конец ремня вокруг стволов. Сама схватилась за ремень, сев на землю и уперевшись ногами."
    ...
    uv_cgl "Собой. Не бойся, это не больно. Ты ничего не потеряешь и ничего не почувствуешь. Просто у тебя появится много сестрёнок. Очень похожих на тебя, но в чем-то других. Совсем большой ты не станешь, но наполовину подрастешь. Где-то до границы детства. Дальше у меня силы нет, дальше — сама."
    us "Я согласна!"
